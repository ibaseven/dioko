...
@Component({
  selector: 'app-crypto-confirm',
  templateUrl: './crypto-confirm.page.html',
  styleUrls: ['./crypto-confirm.page.scss'],
})

export class CryptoConfirmPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.donnees.service_1 == 'Wallet') {
      this.form = new FormGroup({
        debit: new FormControl(this.donnees.wallet),
        provenance: new FormControl(this.donnees.numero),
        beneficiaire: new FormControl(this.donnees.adresse),
        cfa: new FormControl(this.donnees.service_1),
        crypto: new FormControl(this.donnees.usdt),
      ...
      this.form = new FormGroup({
        ...
        provenance: new FormControl(this.donnees.adresse),
        beneficiaire: new FormControl(this.donnees.numero),
        cfa: new FormControl(this.donnees.service_1),
        crypto: new FormControl(this.donnees.usdt),
        service: new FormControl(this.donnees.wallet),
        step: new FormControl(0),
        trans_id: new FormControl(''),
        otp: new FormControl(''),
        validation_otp: new FormControl(''),
   ...
   ionViewWillEnter() {
    this.class1 = 'test';
    this.loader = true;
    this.reqService2.CryptoValue().subscribe(
      async (data: string) => {
      ...
      await loading.present();
        ...
        if((this.form.value.debit == "Orange Money" || this.form.value.debit == "Orange Money BF" || this.form.value.debit == "Orange Money ML" || this.form.value.debit == "Orange Money CI") && this.code_vali == null){
          ...
          await loading.present();
              ...
              this.reqService2.Transfert(this.form.value).subscribe(
                async (data) => {
